    ...
    return (
      <div className="max-w-xl">
        <Card className="border-red-200 bg-red-50">
          <CardContent className="p-6 text-red-700 text-sm font-semibold">{error}</CardContent>
        </Card>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        <KpiCard label="Monthly recurring revenue" value={money(t.mrr)} sub={`${t.payingUsers} paying users`}
          icon={<TrendingUp className="w-5 h-5 text-emerald-600" />} accent="text-emerald-600" />
        <KpiCard label="Revenue all-time" value={money(t.revenueAllTime)}
          sub={`${money(t.revenueThisMonth)} this month`} icon={<Wallet className="w-5 h-5 text-blue-600" />} />
        <KpiCard label="Total users" value={t.users.toLocaleString()}
          sub={`${t.usersThisMonth} new this month`} icon={<Users className="w-5 h-5 text-indigo-600" />} />
        <KpiCard label="Leads captured" value={t.leadsTotal.toLocaleString()}
          sub={`${t.leadsNew} new · ${t.conversionRate}% converted`} icon={<Target className="w-5 h-5 text-rose-600" />} />
        <KpiCard label="Posts today" value={t.postsToday.toLocaleString()}
          sub={`${t.postsAllTime.toLocaleString()} all-time`} icon={<Activity className="w-5 h-5 text-purple-600" />} />
        <KpiCard label="GBP connected" value={t.gbpConnected.toLocaleString()}
          sub={`${t.agencyClients} agency traders`} icon={<Globe className="w-5 h-5 text-cyan-600" />} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2 border-slate-200 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-black text-slate-900">Revenue — last 6 months</CardTitle>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-black text-slate-900">Users by plan</CardTitle>
        ...
        <Card className="lg:col-span-3 border-slate-200 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-black text-slate-900">New signups — last 14 days</CardTitle>
            <CardDescription>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="pb-2 flex-row items-center justify-between space-y-0">
            <CardTitle className="text-base font-black text-slate-900">Recent transactions</CardTitle>
          ...
          <CardContent className="px-2 pb-3">
            {data.recentTransactions.length === 0 ? (
              <p className="text-sm text-slate-400 font-medium px-4 py-6 text-center">
                ...
                {data.recentTransactions.map((tr) => (
                  <li key={tr.id} className="flex items-center justify-between gap-3 px-4 py-2.5">
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-slate-800 truncate">{tr.email || "—"}</p>
                      <p className="text-[11px] font-medium text-slate-400 mt-0.5">
                        {(tr.event_type || "transaction").replace(".", " · ")} {tr.plan ? `· ${tr.plan}` : ""}
                      </p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className={`text-sm font-black tabular-nums ${Number(tr.amount) < 0 ? "text-red-600" : "text-slate-900"}`}>
                        {tr.amount === null ? "—" : money(Number(tr.amount))}
                      </p>
                      <p className="text-[10px] font-medium text-slate-400">{fmtDate(tr.occurred_at)}</p>
                    </div>
                  </li>
                ))}
              </ul>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="pb-2 flex-row items-center justify-between space-y-0">
            <CardTitle className="text-base font-black text-slate-900">Recent leads</CardTitle>
          ...
          <CardContent className="px-2 pb-3">
            {data.recentLeads.length === 0 ? (
              <p className="text-sm text-slate-400 font-medium px-4 py-6 text-center">
                No leads yet. Wire your Facebook / Instagram / Google Ads forms to <code className="text-emerald-700">POST /api/leads/submit</code>.
              </p>
            ) : (
              <ul className="divide-y divide-slate-100">
                {data.recentLeads.map((l) => (
                  <li key={l.id} className="flex items-center justify-between gap-3 px-4 py-2.5">
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-slate-800 truncate">{l.name || l.business_name || l.email || "—"}</p>
                      <p className="text-[11px] font-medium text-slate-400 mt-0.5 truncate">{l.phone || l.email || "No contact"}</p>
                    </div>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="pb-2 flex-row items-center justify-between space-y-0">
            <CardTitle className="text-base font-black text-slate-900">Latest activity</CardTitle>
          ...
          <CardContent className="px-2 pb-3">
            {data.recentActivity.length === 0 ? (
              <p className="text-sm text-slate-400 font-medium px-4 py-6 text-center">No posts generated yet.</p>
            ) : (
              <ul className="divide-y divide-slate-100">
                {data.recentActivity.map((a) => (
                  <li key={a.id} className="flex items-start gap-3 px-4 py-2.5">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                      a.source === "whatsapp" ? "bg-[#dcfce7] text-[#128C7E]" : "bg-blue-50 text-blue-600"
                    }`}>
                      {a.source === "whatsapp" ? <MessageSquare className="w-4 h-4" /> : <Smartphone className="w-4 h-4" />}
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-slate-800 truncate">{a.author}</p>
                      <p className="text-[11px] text-slate-500 line-clamp-2 mt-0.5 leading-snug">{a.text || "Media post"}</p>
                      <p className="text-[10px] font-medium text-slate-400 mt-1">{fmtDate(a.date)} · {a.time}</p>
                    </div>
                  </li>
                ))}
              </ul>
