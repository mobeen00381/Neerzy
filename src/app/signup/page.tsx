...
function SignupContent() {
  ...
  // Step 1: Send OTP via WhatsApp (Twilio Verify)
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone, otp, plan }),
      });
      const data = await res.json();
      
      if (res.ok) {
        // Redirect to onboarding
        router.push(`/onboarding?phone=${encodeURIComponent(phone)}&plan=${plan}`);
      } else {
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl p-10 border border-emerald-100/20">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500 font-medium leading-relaxed">
              {step === 'phone' 
                ? 'The WhatsApp-first marketing platform for local businesses.' 
          ...
          {step === 'phone' ? (
            <form onSubmit={handleSendOtp} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white font-black py-4 rounded-2xl shadow-lg transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 group"
              >
            ...
            <form onSubmit={handleVerify} className="space-y-6">
              <div className="space-y-2 text-center mb-4">
                <p className="text-sm text-slate-500">Sent to <span className="font-bold text-slate-900">{phone}</span></p>
              </div>
              <div className="relative group">
              ...
              <div className="space-y-3">
                <button
                  ...
                  className="w-full bg-[#0F5C4D] hover:bg-[#073a30] text-white font-black py-4 rounded-2xl shadow-lg transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2"
                >
